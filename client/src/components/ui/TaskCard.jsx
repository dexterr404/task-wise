...
function TaskCard({task,team}){
    ...
    return<div className="flex items-center gap-4 bg-bg cursor-pointer hover:-translate-y-1 transform transition-transform ease-in-out duration-200 shadow-md p-4 rounded-md max-lg:flex-col max-lg:items-start relative">
        <div className="flex flex-col">
            <h1 className="text-text-primary lg:w-[260px] text-sm break-words">{task.title}</h1>
            <p className="text-xs text-text-secondary lg:w-[260px]">{task.description}</p>
        </div>
        ...
        <div className="flex gap-2 flex-1 max-lg:w-full">
            <span className="flex items-center text-xs gap-1 text-text-primary"><TimeIcon sx={{ fontSize: 18 }}/>
            {`${countRemainingDays(task.deadline)}`}
            </span>
            <LinearWithValueLabel task={task} className='flex-1'/>
        </div>
        <div className="absolute bottom-14 right-4 lg:top-0.5">
          {isTeamsPage && (
            <AvatarGroup max={4}>
        ...
        <div className="max-lg:absolute max-lg:top-2 max-lg:right-2 z-10">
          <div className="relative inline-flex">
            <Tooltip title="Options">
              <IconButton sx={{ padding: "1px", color: "var(--color-text-primary)"}}>
                <MoreHorizIcon
